...
async function runSeed() {
  ...
  try {
    ...
    for (const sheetName of pdvWb.SheetNames) {
      ...
      // Obtener nombre de PDV del título o nombre de pestaña
      let pdvNombre = (rows[0][0] || sheetName).toString().trim();
      if (!pdvNombre) pdvNombre = sheetName;

      // Insertar o actualizar Sede PDV
      ...
      // Encontrar fila de encabezados (usualmente fila 1 o 2)
      let headerRowIdx = -1;
      ...
      // Procesar filas de datos del PDV
      for (let r = headerRowIdx + 1; r < rows.length; r++) {
        ...
        // 3.1 Equipo Principal (PC / All In One)
        ...
        if (monitorRaw && monitorRaw !== 'N/A') {
          ...
          await client.query(`
            INSERT INTO equipos (
              modulo, modelo, tipo_equipo_id, marca_id, sede_id, estado_id,
              equipo_padre_id, created_by
            ) VALUES ('PDV', $1, $2, $3, $4, 1, $5, 'SEEDER');
          `, [monitorRaw, getTipoId('Monitor'), marcaId, sedeId, pcEquipoId]);
          totalPdvEquipos++;
        }

        // 3.3 Periféricos: Teclado y Mouse
        ...
        // 3.5 Cajón Monedero, Báscula, Modem, TV, UPS
    ...
    for (const row of tiRows) {
      ...
      const insSede = await client.query(`
        INSERT INTO sedes (nombre, tipo, ciudad)
        VALUES ($1, 'SEDE_ADMINISTRATIVA', 'Bogotá')
        ON CONFLICT (nombre) DO UPDATE SET nombre = EXCLUDED.nombre
        RETURNING id;
      `, [sedeNombre]);
      const sedeId = insSede.rows[0]?.id;
      ...
      if (estadoRaw.toUpperCase().includes('LENT') || estadoRaw.toUpperCase().includes('MANCHA') || estadoRaw.toUpperCase().includes('MOLESTA')) {
        estadoId = 2; // Lentitud / Observación
      }
      ...
      // Periférico: Monitor si viene reportado
    ...
    // 5. Mantenimientos de ejemplo y evaluación de alertas
    console.log('5. Programando mantenimientos preventivos y evaluando alertas iniciales...');
    ...
    for (let i = 0; i < randomEquipos.rows.length; i++) {
      ...
      const days = (i % 3 === 0) ? -5 : (i * 10 + 5); // algunos vencidos y otros próximos
      await client.query(`
    ...
    // Simular un usuario inactivo para probar regla 3.4
    await client.query(`
